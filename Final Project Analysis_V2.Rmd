...
This analysis uses the "Wine Quality" dataset from the UCI Machine Learning Repository, which contains physicochemical properties and quality ratings for red wine variants of Portuguese "Vinho Verde" wine. The dataset includes:
...
# Packages used in assessment files:
library(car)       # For VIF analysis (Assessment 3)
library(lmtest)    # For Breusch-Pagan test (Assessment 4)
library(sandwich)  # For robust standard errors (Assessment 4)
library(MASS)      # For stepwise selection (Assessments 5 & 6)
library(lmboot)    # For bootstrap confidence intervals (Assessment 6)
```
...
hist(wine_data$quality, 
     ...
     col = "pink",
     breaks = 10)
abline(v = mean(wine_data$quality), col = "blue", lwd = 2, lty = 2)
...
```{r}
# Diagnostic plots for linearity assessment
par(mfrow = c(2, 2))
plot(initial_model)
par(mfrow = c(1, 1))
...
qqnorm(initial_model$residuals, main = "Normal Q-Q Plot of Residuals", 
       pch = 19, col = "pink")
abline(0, 1, col = "blue", lwd = 2, lty = 2)  # Using abline instead of qqline
...
All VIF values are below 2, indicating that multicollinearity is not a concern in this model.
